<template>
  <div class="p-4 bg-white dark:bg-[#1e293b] rounded-lg shadow-sm transition-colors duration-300 relative">
    ...
    <div
      ...
      @click.stop
    >
      <button class="ctx-item" @click="onContextToggleEnable">{{ contextMenuRow?.status === 1 ? '禁用账户' : '启用账户' }}</button>
      <button class="ctx-item" @click="onContextEdit">编辑用户</button>
      <button class="ctx-item" @click="onContextOpenRoleDialog">修改角色</button>
      <button class="ctx-item ctx-item--danger" @click="onContextDelete">删除</button>
    </div>
...
import { ref, reactive, onMounted, onUnmounted, nextTick, type CSSProperties } from 'vue'
